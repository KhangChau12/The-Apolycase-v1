...
const STAT_UPGRADES: StatUpgradeDef[] = [
  {
    key: 'maxHp',
    label: 'Max HP',
    desc: '+30 max health, heals 12',
    icon: 'heart',
    baseCost: 35,
    maxPurchases: 8,
    apply: (p) => { p.stats.maxHp += 30; p.stats.hp = Math.min(p.stats.hp + 12, p.stats.maxHp) },
  },
  {
    key: 'damage',
    label: 'Damage',
    desc: '+6 bullet damage',
    icon: 'zap',
    baseCost: 45,
    maxPurchases: 6,
  ...
  },
  {
    ...
    baseCost: 55,
    maxPurchases: 6,
    apply: (p) => { p.stats.critChance = Math.min(0.8, p.stats.critChance + 0.04) },
  },
  {
    key: 'pickupRange',
    label: 'Pickup Range',
    desc: '+25 auto-collect radius',
    icon: 'expand',
    baseCost: 25,
    maxPurchases: 4,
    apply: (p) => { p.stats.pickupRange += 25 },
  },
]
